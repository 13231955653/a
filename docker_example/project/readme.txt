路由  自动加载  命名空间
php js html css css3 supervisord redis集群 curl frp nginx集群  Percona XtraDB
 Cluster(PXC) 5.7 pdo预处理 rsa sql日志 仿laravel artsian/command命令 model command 生成
 前置/后置中间件 钩子 laravel  查询构造器/Query Builder
 控制反转 依赖注入 对象池模式
 mysql集群  多例模式
数据库避免死锁，使用非主键索引更新或删除，会先锁非主键索引，然后再锁主键索引。如果在锁完非主键索引后再锁定主键索引前有其他操作锁住了该主键索引，就会造成死锁。
更新时使用主键索引，没有主键索引，可先查询出来。

   路由
多语言
请求频率限制



中文字符串切割  中文添加拼音  拼音切割


需

入口文件限制请求频率

docker exec -it project-slove-10013 /usr/bin/supervisord -c /etc/supervisor/supervisord.conf

apt install php7.4-fpm php7.4-dev php7.4-curl php7.4-mbstring  php7.4-redis  php-pear  -y

后台菜单无极限




