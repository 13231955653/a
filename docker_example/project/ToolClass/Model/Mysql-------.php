...
//    public $sDisposeIncrbyDecrbySql           = '';
    ...
    function __construct ($sTable = '')
    {
        ...
        $this->sNowTable = SqlSafe::filterSqlArg( $sTable);
...
//        $this->sDisposeIncrbyDecrbySql = '';
    ...
    function where (
        ...
        foreach ( $aWhere as $k => $v ) {
            $this->aWhereConditions[ $k ] = SqlSafe::filterSqlArg( $v );
        ...
        if ($aData) {
            if (is_array($aData)) {
                foreach ($aData as $k => $v) {
                    $aData[$k] = SqlSafe::filterSqlArg( $v );
                }
                ...
                $aData = '`' . SqlSafe::filterSqlArg( $aData ) . '`';
            }
        }
...
//            foreach ($v1 as $k => $v) {
                ...
                $this->aUpdateData[ $k1 ] = SqlSafe::filterSqlArg( $v1 );
//            }
        }
