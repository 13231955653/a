...
class CurlTool extends ToolFather
{
     ...
     */
    public
    function __construct ($url, $responseHeader = 0)
    {
     ...
     */
    public static
    function instance ($url, $responseHeader = 0)
    {
     ...
     */
    public
    function post ($aValue, $https = TRUE)
    {
        if ($https) {
...
//        curl_setopt($this->ch, CURLOPT_POSTFIELDS, $aValue);
        curl_setopt($this->ch, CURLOPT_POSTFIELDS, http_build_query($aValue));

        return $this->exec();
    }

    private
    function addVerifyData (& $aValue)
    {
