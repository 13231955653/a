...
class ChineseStringTool extends ToolFather
{
    ...
    function insert (
        ...
        $aFontData                         = [];
        $aFontData[ $sWordFeild ]          = trim(
            $aChineseString[ $sWordFeild ]
        );
        $aFontData[ $sWordUrlencodeFeild ] = JianFanFontModel::urlencode(
            $aFontData[ $sWordFeild ]
        );
        $aFontData[ $sWordUrlencodeMd5Feild ]
                                           = JianFanFontModel::wordUrlencodeMd5Encode(
            $aFontData[ $sWordFeild ]
        );
        if ( ChineseStringModelTool::checkExistSmaeMd5(
            $aFontData[ $sWordUrlencodeMd5Feild ]
        ) ) {
            //            var_dump('word exist !!!!');
            $aData = NULL;
        ...
        if ( isset( $aChineseString[ $sOldWordFeild ] )
             && $aChineseString[ $sOldWordFeild ] ) {
            $aFontData[ $sOldWordFeild ] = trim(
                $aChineseString[ $sOldWordFeild ]
            );
        }

        if ( isset( $aChineseString[ $sOldWordUrlencodeFeild ] )
             && $aChineseString[ $sOldWordUrlencodeFeild ] ) {
            $aFontData[ $sOldWordUrlencodeFeild ] = trim(
                $aChineseString[ $sOldWordUrlencodeFeild ]
            );
        } else {
            if ( isset( $aChineseString[ $sOldWordFeild ] ) ) {
                $aFontData[ $sOldWordUrlencodeFeild ]
                    = JianFanFontModel::urlencode(
                    $aChineseString[ $sOldWordFeild ]
                );
            }
        }

        if ( isset( $aChineseString[ $sOldWordUrlencodeMd5Feild ] )
             && $aChineseString[ $sOldWordUrlencodeMd5Feild ] ) {
            $aFontData[ $sOldWordUrlencodeMd5Feild ] = trim(
                $aChineseString[ $sOldWordUrlencodeMd5Feild ]
            );
        } else {
            if ( isset( $aChineseString[ $sOldWordFeild ] ) ) {
                $aFontData[ $sOldWordUrlencodeMd5Feild ]
                    = JianFanFontModel::wordUrlencodeMd5Encode(
                    $aChineseString[ $sOldWordFeild ]
                );
            }
        ...
        if ( isset( $aChineseString[ $sStrokesFeild ] )
             && $aChineseString[ $sStrokesFeild ] ) {
            $aFontData[ $sStrokesFeild ] = trim(
                $aChineseString[ $sStrokesFeild ]
            );
        }

        if ( isset( $aChineseString[ $sPinyinWithVoiceFeild ] )
             && $aChineseString[ $sPinyinWithVoiceFeild ] ) {
            $aFontData[ $sPinyinWithVoiceFeild ] = trim(
                $aChineseString[ $sPinyinWithVoiceFeild ]
            );
        } else {
            $aFontData[ $sPinyinWithVoiceFeild ] = trim(
                PinYinShengDiao::TransformWithTone(
                    $aChineseString[ $sWordFeild ]
                )
            );
            //            var_dump($aFontData[ $sPinyinWithVoiceFeild ], $aFontData[ $sWordFeild ], $aFontData[ $sPinyinWithVoiceFeild ] === $aFontData[ $sWordFeild ]);
            //            die();
            if ( !$aFontData[ $sPinyinWithVoiceFeild ]
                 || JianFanFontModel::urlencode(
                    $aFontData[ $sPinyinWithVoiceFeild ]
                ) === $aFontData[ $sWordUrlencodeFeild ] ) {
                unset( $aFontData[ $sPinyinWithVoiceFeild ] );
            }
        }

        if ( isset( $aChineseString[ $sPinyinFeild ] )
             && $aChineseString[ $sPinyinFeild ] ) {
            $aFontData[ $sPinyinFeild ] = trim(
                $aChineseString[ $sPinyinFeild ]
            );
        } else {
            $aFontData[ $sPinyinFeild ] = trim(
                PinYinShengDiao::TransformWithoutTone(
                    $aChineseString[ $sWordFeild ]
                )
            );
            if ( !$aFontData[ $sPinyinFeild ]
                 || JianFanFontModel::urlencode(
                    $aFontData[ $sPinyinFeild ]
                ) === $aFontData[ $sWordUrlencodeFeild ] ) {
                unset( $aFontData[ $sPinyinFeild ] );
            }
        }

        if ( isset( $aChineseString[ $sFirstPinyinFeild ] )
             && $aChineseString[ $sFirstPinyinFeild ] ) {
            $aFontData[ $sFirstPinyinFeild ] = trim(
                $aChineseString[ $sFirstPinyinFeild ]
            );
        } else {
            $aFontData[ $sFirstPinyinFeild ] = trim(
                PinYinShengDiao::TransformUcwords(
                    $aChineseString[ $sWordFeild ],
                    FALSE
                )
            );
            if ( !$aFontData[ $sFirstPinyinFeild ]
                 || JianFanFontModel::urlencode(
                    $aFontData[ $sFirstPinyinFeild ]
                ) === $aFontData[ $sWordUrlencodeFeild ] ) {
                unset( $aFontData[ $sFirstPinyinFeild ] );
            }
        }

        if ( isset( $aChineseString[ $sRadicalsFeild ] )
             && $aChineseString[ $sRadicalsFeild ] ) {
            $aFontData[ $sRadicalsFeild ] = trim(
                $aChineseString[ $sRadicalsFeild ]
            );
        }

        if ( isset( $aChineseString[ $sExplanationFeild ] )
             && $aChineseString[ $sExplanationFeild ] ) {
            $aFontData[ $sExplanationFeild ] = self::replaceChineseString(
                $aChineseString[ $sExplanationFeild ],
                $aFontData[ $sWordFeild ]
            );
            $aFontData[ $sExistExplain ]
                                             = JianFanFontModel::existExplainTag(
            );
        }

        if ( isset( $aChineseString[ $sMoreFeild ] )
             && $aChineseString[ $sMoreFeild ] ) {
            $aFontData[ $sMoreFeild ] = self::replaceChineseString(
                $aChineseString[ $sMoreFeild ],
                $aFontData[ $sWordFeild ]
            );
        ...
        Hook::addHook(
            'string\ChineseFont\ChineseFont',
            'afterUpdateChineseFont'
        );
        //            更新文字拼音 cache hook
        Hook::listen(
            Hook::chineseFont(),
    ...
    function replaceChineseString (
        ...
        $sDependName = DependContainer::regularVerify();
        Ioc::register($sDependName, DependContainer::depend( $sDependName));
        $oRegularVerify = Ioc::resolve($sDependName);

        $sString = $oRegularVerify->disposeLineFeed( $sString );
        ...
        $sDependName = DependContainer::string();
        Ioc::register($sDependName, DependContainer::depend( $sDependName));
        $oString = Ioc::resolve($sDependName);
        $sString = $oString->replaceEnglishSymbolToChineseSymbol( $sString );
    ...
    function searchString (
        ...
        switch ( $sSearchWhat ) {
            //            不存在繁体字
            case 'noOld' :
                $aWhere[ JianFanFontModel::oldWordUrlencodeMd5() ]
                    = JianFanFontModel::noExistOldWordTag();
                break;
            //            不存在笔画
            case 'noStrokes' :
                $aWhere[ JianFanFontModel::strokes() ]
                    = JianFanFontModel::noStrokes();
                break;
            //            不存在拼音
            case 'noPinYin' :
                $aWhere[ JianFanFontModel::firstPinyin() ]
                    = JianFanFontModel::noPinYin();
                break;
            //            不存在偏旁
            case 'noRadicals' :
                $aWhere[ JianFanFontModel::radicals() ]
                    = JianFanFontModel::noRadicals();
                break;
            //            不存在说明
            case 'noExplain' :
                $aWhere[ JianFanFontModel::existExplanation() ]
                    = JianFanFontModel::noExistExplanation();
                break;
            //            不存在更多说明
            case 'noMore' :
                $aWhere[ JianFanFontModel::existMore() ]
                    = JianFanFontModel::noMore();
                break;
            default :
                Exception::throwException(
                    Server::response(
        ...
        return ChineseStringModelTool::getJianFanZiFromDatabase(
            $aWhere,
            $iGetNum
        );
    }
