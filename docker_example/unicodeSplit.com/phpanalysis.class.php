...
//            if (!$this->mainDicHand) {
//                $this->mainDicHand = $redis->get(REDIS_PREFIX . MASTER_DIC);
//                $this->mainDicHand = json_decode($this->mainDicHand);
//                if (!$this->mainDicHand) {
//                    //加载主词典（只打开）
//                    $this->mainDicHand = fopen($this->mainDicFile, 'r');
//                    $redis->set(REDIS_PREFIX . MASTER_DIC, json_encode($this->mainDicHand));
//                }
//            }
//        } else {
//            $this->mainDicHand = fopen($this->mainDicFile, 'r');
...
//                echo 1;
//                var_dump(file($dicWords));
//                $redis->set(REDIS_PREFIX . MASTER_DIC, json_encode(file($dicWords)));
//            }
//
//            $ds = $redis->get(REDIS_PREFIX . VICE_DIC);
//            $ds = json_decode($ds);
//            if (!$ds) {
//                //载入副词典
//                $ds = file($dicAddon);
//                $redis->set(REDIS_PREFIX . VICE_DIC, json_encode($ds));
//            }
//        } else {
            //加载主词典（只打开）
            $this->mainDicHand = fopen($dicWords, 'r');
            ...
            if ($slen < $this->notSplitLen) {
                ...
                if ($slen < 5) {
                    ...
                    if (
                        ...
                        &&
                        (
                            isset($this->addonDic['u'][$str])
                            ||
                            isset($this->addonDic['u'][substr($str, 0, 2)])
                        )
                    ) {
                        $str2 = '';
                        if (
                            !isset($this->addonDic['u'][$str])
                            &&
                            isset($this->addonDic['s'][substr($str, 2, 2)])
                        ) {
                            $str2 = substr($str, 2, 2);
                            $str  = substr($str, 0, 2);
                        }
        ...
        if ($prePos > -1 && !isset($this->finallyResult[$prePos])) {
            ...
            if (
                (
                    $lastt==4
                    ||
                    isset($this->addonDic['c'][$lastw])
                )
                &&
                isset($this->addonDic['u'][$smarr[0]])
            ) {
            ...
            if (isset($this->addonDic['c'][$cw]) && isset($this->addonDic['u'][$nw])) {
                ...
                if (
                    ...
                    if (
                        ...
                        strlen($smarr[$i+2]) == 2
                        &&
                        !isset($this->addonDic['s'][$smarr[$i+2]])
                    ) {
                        $newarr[$j] .= $smarr[$i+2];
                        $i++;
                    }
                ...
                if (
                    ...
                    strlen($nw) == 2
                    &&
                    !isset($this->addonDic['s'][$cw])
                    &&
                    !isset($this->addonDic['t'][$cw])
                    &&
                    !isset($this->addonDic['a'][$cw])
                    &&
                    !isset($this->addonDic['s'][$nw])
                    &&
                    ...
                    if (
                        ...
                        &&
                        (
                            isset($this->addonDic['a'][$smarr[$i+2]])
                            ||
                            isset($this->addonDic['u'][$smarr[$i+2]])
                        )
                    ) {
                        $newarr[$j] .= $smarr[$i+2];
                        $i++;
...
//        }
//        return $rsstr;
        switch ($this->_source_result_charset()) {
            case '1' :
                $rsstr = iconv(UCS2, 'utf-8', $str);
            break;
            case '2' :
     ...
     */
     public function GetFinallyResult($spword=' ', $word_meanings=false)
     {
        ...
        $rsstr = mb_substr($rsstr, mb_strlen($spword));
        return $rsstr;
     }
